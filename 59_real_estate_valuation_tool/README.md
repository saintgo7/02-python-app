...
**59_real_estate_valuation_tool**는 Real estate property valuation tool
...
#### 1단계: 저장소 클론
```bash
cd 59_real_estate_valuation_tool
```
...
```bash
# 이미지 빌드
docker build -t 59_real_estate_valuation_tool .

# 컨테이너 실행
docker run -p 8000:8000 59_real_estate_valuation_tool
```
