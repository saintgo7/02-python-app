...
**55_linkedin_profile_analyzer**는 LinkedIn profile analysis tool
...
#### 1단계: 저장소 클론
```bash
cd 55_linkedin_profile_analyzer
```
...
```bash
# 이미지 빌드
docker build -t 55_linkedin_profile_analyzer .

# 컨테이너 실행
docker run -p 8000:8000 55_linkedin_profile_analyzer
```
