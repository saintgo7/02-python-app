...
**23_image_classification_service**는 Image classification API
...
#### 1단계: 저장소 클론
```bash
cd 23_image_classification_service
```
...
```bash
# 이미지 빌드
docker build -t 23_image_classification_service .

# 컨테이너 실행
docker run -p 8000:8000 23_image_classification_service
```
