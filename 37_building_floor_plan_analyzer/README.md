...
**37_building_floor_plan_analyzer**는 Floor plan recognition and analysis
...
#### 1단계: 저장소 클론
```bash
cd 37_building_floor_plan_analyzer
```
...
```bash
# 이미지 빌드
docker build -t 37_building_floor_plan_analyzer .

# 컨테이너 실행
docker run -p 8000:8000 37_building_floor_plan_analyzer
```
