...
**60_invoice_processing_system**는 Automated invoice processing system
...
#### 1단계: 저장소 클론
```bash
cd 60_invoice_processing_system
```
...
```bash
# 이미지 빌드
docker build -t 60_invoice_processing_system .

# 컨테이너 실행
docker run -p 8000:8000 60_invoice_processing_system
```
