...
**53_report_automation_system**는 Automated report generation system
...
#### 1단계: 저장소 클론
```bash
cd 53_report_automation_system
```
...
```bash
# 이미지 빌드
docker build -t 53_report_automation_system .

# 컨테이너 실행
docker run -p 8000:8000 53_report_automation_system
```
