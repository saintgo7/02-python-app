...
**38_wildlife_species_detector**는 Wildlife species identification system
...
#### 1단계: 저장소 클론
```bash
cd 38_wildlife_species_detector
```
...
```bash
# 이미지 빌드
docker build -t 38_wildlife_species_detector .

# 컨테이너 실행
docker run -p 8000:8000 38_wildlife_species_detector
```
