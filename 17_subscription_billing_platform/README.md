...
**17_subscription_billing_platform**는 Subscription and billing management system
...
#### 1단계: 저장소 클론
```bash
cd 17_subscription_billing_platform
```
...
```bash
# 이미지 빌드
docker build -t 17_subscription_billing_platform .

# 컨테이너 실행
docker run -p 8000:8000 17_subscription_billing_platform
```
